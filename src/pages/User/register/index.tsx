...
const Register: FC = () => {
  ...
  const onFinish = (values: Store) => {
    handleRegister(values as API.LoginRegisterDTO).then((result) => {
      if (result.code === '0000') {
        message.success('注册成功！');
  ...
  const handleRegister = async (values: API.LoginRegisterDTO) => {
    return await userRegisterUsingPost(values);
  };
